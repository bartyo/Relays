...
void loop() {
  ...
  if(g_token_send_HTTP == 0 && g_token_send_BLE == 0 && Serial.available()>0)
  {
    g_char_serial_read = (char)Serial.read();
  }
  read_http();
  ...
  if(millis()-timer > BLE_TIME)
  {
    send_ble();
  }

  if(millis()-timer > CHECK_ALIVE_TIME)
  {
    check_alive();
  }
...
void read_http()//
{
  if(g_char_serial_read == '/'){display(g_buffer_http);}
  if(g_char_serial_read == BEGIN_HTTP)
  {
    //Serial.print("http debut");
    reset_buffer(g_buffer_http);
    g_begin_reception_HTTP = 1;
    g_end_reception_HTTP = 0;
  }
  if(g_char_serial_read == END_HTTP)
  {
    //Serial.print("http fin");
    g_begin_reception_HTTP = 0;
    g_end_reception_HTTP = 1;
  }
  if(g_char_serial_read != BEGIN_HTTP && g_char_serial_read != END_HTTP && g_begin_reception_HTTP == 1 && g_end_reception_HTTP == 0 && g_char_serial_read != 0)
  {
    g_buffer_http[g_char_loop] = g_char_serial_read;
    g_char_loop++;
  }
...
void read_ble()
{
  if(g_char_serial_read == '&'){display(g_buffer_ble);}
  if(g_char_serial_read == BEGIN_BLE)
  {
    //Serial.print("ble debut");
    reset_buffer(g_buffer_ble);
    g_begin_reception_BLE = 1;
    g_end_reception_BLE = 0;
  }
  if(g_char_serial_read == END_BLE)
  {
    //Serial.print("ble fin");
    g_begin_reception_BLE = 0;
    g_end_reception_BLE = 1;
  }
  if(g_char_serial_read != BEGIN_BLE && g_char_serial_read != END_BLE && g_begin_reception_BLE == 1 && g_end_reception_BLE == 0 && g_char_serial_read != 0)
  {
    
    g_buffer_ble[g_char_loop] = g_char_serial_read;
    g_char_loop++;
    //Serial.print(g_char_loop,DEC);
  }
